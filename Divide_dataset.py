...
def div_train_val (num_split, train_size, val_size, img_depth, img_size, num_channels):

   X=np.load(X.npy)
   Y=np.load(Y.npy)

   X_train=np.zeros((train_size,img_depth, img_size,img_size,num_channels))
   X_val=np.zeros((val_size, img_depth, img_size,img_size,num_channels))

   ytrain=np.zeros((train_size))
   yval=np.zeros((val_size))
